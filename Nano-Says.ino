// Based on https://www.instructables.com/id/Arduino-Simon-Says-1/
...
void readSequence(){
  ...
  if(button_value > 0){
    ...
    if(button_value == GAME_SEQUENCE[READ_STEP]){
      ...
      // Lets speed it up if not too fast already!
      if(GAME_SPEED > 200) GAME_SPEED = GAME_SPEED-15;
...
void gameOver(){
  ...
  // Light all LEDs to indicate restart
  setLEDs(true,true,true,true);
  delay(GAME_SPEED);
  setLEDs(false,false,false,false);
